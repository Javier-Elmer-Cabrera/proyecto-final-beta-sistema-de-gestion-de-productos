...
public class StockQueryPanel extends AbstractFunctionPanel {
    ...
    private void intCombo() {
        ...
        // DISEÑO UI: Listener estético que actualiza el panel de especificaciones según la
        // categoría seleccionada en el combo. No altera las estructuras internas del ABB ni la cola.
    ...
    private JPanel getButtonPanel() {
        if (buttonPanel == null) {
            ...
            JButton btnSaveToExcel = new JButton("Guardar en Excel");
            btnSaveToExcel.addActionListener(e -> {
                ...
                try {
                    ...
                    JOptionPane.showMessageDialog(null, "No se pudo guardar: " + e1.getMessage());
                }
            });
    ...
    private JPanel getUpperFormPanel() {
        if (formPanel == null) {
            ...
            formPanel.setBorder(new TitledBorder(null, "Búsqueda de Inventario", TitledBorder.LEADING, TitledBorder.TOP, null, null));
            ...
            JLabel lblItemName = new JLabel("Nombre del Artículo");
            ...
            JLabel lblN = new JLabel("Categoría");
...
//					itemReceiverPanel.clearAll();
    ...
    // REQUISITO: Consulta de stock mediante filtrado funcional sobre la lista obtenida del
    // BinaryProductTree (recorrido Inorden O(n)). Cada filtro se aplica en secuencia
    // sobre el Stream de Items activos para localizar coincidencias por nombre, categoría,
    // proveedor, ubicación, rango de fechas y especificaciones dinámicas.
    private void readAndShowAll() {
        try {
            ...
            brsL = MemoryRepository.getInstance().itemStockQuery(txtItemname.getText(), cmbCategory.getSelectedId(), cmbVendor.getSelectedId(), txtRacknumber.getText().trim(),
    ...
    private void showListInGrid(List<Item> brsL) {
        ...
        for (Item bo : brsL) {
            ...
            if (bo.getAddedType() == Item.ADD_TYPE_NEW_ENTRY) {
                addedType = "Stock de Compra";
            } else {
                addedType = "Stock Devuelto";
            }
