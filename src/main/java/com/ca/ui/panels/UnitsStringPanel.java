...
public class UnitsStringPanel extends AbstractFunctionPanel {
    private final String[] header = new String[]{"N.°", "ID", "Nombre", "Fecha de Creación"};
    ...
    private void deleteSelectedRecord() {
        try {
            MemoryRepository.getInstance().deleteById(UnitsString.class, editingPrimaryId);
            changeStatus(Status.READ);
    ...
    private void initValidator() {
        ...
        v.addTask(nameFLD, "Requerido", null, true);
    ...
    private void save(boolean isModified) {
        ...
        if (v.validate()) {
            try {
                ...
                if (isModified) {
                    UnitsString bo = (UnitsString) MemoryRepository.getInstance().getById(UnitsString.class, editingPrimaryId);
    ...
    private JPanel getUpperFormPanel() {
        if (formPanel == null) {
            ...
            formPanel.setBorder(new TitledBorder(null, "Ingreso de Unidades", TitledBorder.LEADING, TitledBorder.TOP, null, null));
    ...
    private void readAndShowAll(boolean showSize0Error) {
        try {
            List<UnitsString> brsL = MemoryRepository.getInstance().readAll(UnitsString.class);
            editingPrimaryId = -1;
    ...
    private void populateSelectedRowInForm(int selectedId) {
        try {
            UnitsString bro = (UnitsString) MemoryRepository.getInstance().getById(UnitsString.class, selectedId);
            if (bro != null) {
