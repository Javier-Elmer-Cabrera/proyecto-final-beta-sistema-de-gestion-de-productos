...
public class ItemEntryPanel extends AbstractFunctionPanel {
    ...
    private void intCombo() {
        ...
        // DISEÑO UI: Listener que actualiza dinámicamente el panel de especificaciones
        // al cambiar la categoría seleccionada. No altera las estructuras de datos.
    ...
    // VINCULACIÓN: Este panel consulta categorías desde el MemoryRepository,
    // el cual a su vez las obtiene del DataEngine (almacenadas en ConcurrentHashMap).
    private void initCmbCategory() throws Exception {
        /* Category Combo */
        cmbCategory.init();
        List<Category> cl = MemoryRepository.getInstance().readAll(Category.class);
        for (Category c : cl) {
    ...
    private void initCmbUnits() throws Exception {
        /* Category Combo */
        cmbUnitcombo.init();
        List<UnitsString> cl = MemoryRepository.getInstance().readAll(UnitsString.class);
    ...
    private void initCmbVendor() throws Exception {
        /* Vendor Combo */
        cmbVendor.init();
        List<Vendor> vl = MemoryRepository.getInstance().readAll(Vendor.class);
        for (Vendor v : vl) {
    ...
    private JPanel getButtonPanel() {
        if (buttonPanel == null) {
            ...
            btnReadAll = new JButton("Leer Todo");
            ...
            JButton btnDeleteThis = new JButton("Eliminar");
    ...
    // REQUISITO: Soft-delete sobre el ABB: marca dFlag=0 sin alterar la estructura del árbol.
    // La búsqueda previa del nodo se realiza en O(log n) por ID.
    private void deleteSelectedItem() {
        try {
            MemoryRepository.getInstance().deleteById(Item.class, editingPrimaryId);
            changeStatus(Status.READ);
    ...
    public final void enableDisableComponents() {
        ...
        switch (status) {
            ...
            case MODIFY:
                ...
                if (currentSpecificationPanel != null) {
                    currentSpecificationPanel.enableAll();
                    forceEnableDynamicTextFields(currentSpecificationPanel);
                }
                txtEntrynumber.setEnabled(false);
                txtEntrynumber.setBackground(new Color(230, 230, 230));
                txtEntrynumber.setForeground(Color.BLACK);
    ...
    private void initValidator() {
        ...
        v = new Validator(mainApp, true);
        v.addTask(txtName, "Requerido", null, true);
        v.addTask(cmbCategory, "Requerido", null, true);
        v.addTask(cmbVendor, "Requerido", null, true);
    ...
    private Item getModelFromForm() {
        Item bo = new Item();
        String entryStr = txtEntrynumber.getText().trim();
        if (!entryStr.isEmpty()) {
            try {
                bo.setId(Integer.parseInt(entryStr));
        ...
        try {
            ...
            bo.setCategory((Category) MemoryRepository.getInstance().getById(Category.class, cmbCategory.getSelectedId()));
            bo.setSpecification(currentSpecificationPanel.getSpecificationsObject());
            bo.setVendor((Vendor) MemoryRepository.getInstance().getById(Vendor.class, cmbVendor.getSelectedId()));
            bo.setUnitsString((UnitsString) MemoryRepository.getInstance().getById(UnitsString.class, cmbUnitcombo.getSelectedId()));
    ...
    private void setModelIntoForm(Item bro) {
        txtEntrynumber.setText(String.valueOf(bro.getId()));
    ...
    // REQUISITO: Inserción y actualización en el BinaryProductTree (ABB) usando MemoryRepository.
    // VINCULACIÓN: Este método envía el objeto Item al DataEngine para que sea indexado
    // en el árbol global (BinaryProductTree) en memoria RAM, ordenado por ID.
    private void save(boolean isModified) {
        ...
        if (v.validate()) {
            ...
            try {
                ...
                if (isModified) {
                    // REQUISITO: Búsqueda binaria O(log n) en el ABB para localizar el Item por ID antes de actualizarlo.
                    Item bo = (Item) MemoryRepository.getInstance().getById(Item.class, editingPrimaryId);
                    ...
                    bo.setSpecification(newBo.getSpecification());
                    bo.setVendor(newBo.getVendor());
                    bo.setUnitsString(newBo.getUnitsString());
                    ...
                    MemoryRepository.getInstance().saveOrUpdate(newBo);
                }
    ...
    private JPanel getUpperFormPanel() {
        if (formPanel == null) {
            ...
            formPanel.setBorder(new TitledBorder(UIManager.getBorder("TitledBorder.border"), "Ingreso de Nuevo Artículo", TitledBorder.LEADING,
            ...
            JLabel lblPurchaseOrderNumber = new JLabel("N.° de Orden de Compra");
            ...
            JLabel lblEntryNumber = new JLabel("N.° de Ingreso");
            ...
            JLabel lblPartsNumber = new JLabel("N.° de Pieza");
            ...
            JLabel lblSerialNumber = new JLabel("N.° de Serie");
            ...
            JLabel lblRacknumber = new JLabel("N.° de Estante");
            ...
            JLabel lblRate = new JLabel("Precio");
            ...
            JLabel lblPurchaseDate = new JLabel("Fecha de Compra");
            ...
            JLabel lblPhoneNumber = new JLabel("Proveedor");
    ...
    // REQUISITO: Recuperación de todos los Items activos mediante recorrido Inorden del ABB,
    // lo que devuelve los productos ordenados ascendentemente por ID (O(n)).
    private void readAndShowAll(boolean showSize0Error) {
        List<Item> brsL = MemoryRepository.getInstance().getAddedItems();
        editingPrimaryId = -1;
    ...
    public final String getFunctionName() {
        return "Ingreso de Nuevo Artículo";
    }
    ...
    private void populateSelectedRowInForm(int selectedId) {
        try {
            Item bro = (Item) MemoryRepository.getInstance().getById(Item.class, selectedId);
    ...
    // ACLARACIÓN: Recorrido recursivo sobre el contenedor de especificaciones en caliente
    // para forzar la mutabilidad de campos dinámicos durante la modificación (MODIFY).
    // Esto es necesario porque Swing deshabilita componentes al hacer toggleAllChildren.
    private void forceEnableDynamicTextFields(Container container) {
        for (Component child : container.getComponents()) {
            ...
            if (child instanceof Container) {
                forceEnableDynamicTextFields((Container) child);
            }
        }
    }
