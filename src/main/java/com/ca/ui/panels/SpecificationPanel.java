...
public class SpecificationPanel extends JPanel {
    ...
    public SpecificationPanel(int id) {
        ...
        addCategoryPanels();
                setBorder(BorderFactory.createTitledBorder("Especificación :  " + category.getCategoryName()));
    }
    ...
    public final void enableAll() {
        for (CategoryEntryPair pair : pairList) {
            pair.tb.setEditable(true);
            pair.tb.setEnabled(true);
            pair.tb.setBackground(Color.WHITE);
        }
    }
    ...
    private void addCategoryPanels() {
        ...
        synchronized (pairList) {
            ...
            if (pairList.size() == 0) {
                ...
                this.removeAll();
                add(new JLabel("No se estableció especificación para " + category.getCategoryName()));
    ...
    private void readList(int id) {
        try {
            category = (Category) MemoryRepository.getInstance().getById(Category.class, id);
        } catch (Exception e) {
