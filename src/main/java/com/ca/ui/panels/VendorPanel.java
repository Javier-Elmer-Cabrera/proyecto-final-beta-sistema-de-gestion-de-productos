...
public class VendorPanel extends AbstractFunctionPanel {
    private final String[] header = new String[]{"N.°", "ID", "Nombre", "Dirección",
            "Teléfono"};
    ...
    private void deleteSelectedVendor() {
        try {
            MemoryRepository.getInstance().deleteById(Vendor.class, editingPrimaryId);
            changeStatus(Status.READ);
    ...
    private void save(boolean isModified) {
        ...
        if (v.validate()) {
            try {
                ...
                if (isModified) {
                    Vendor bo = (Vendor) MemoryRepository.getInstance().getById(Vendor.class,
                            editingPrimaryId);
    ...
    private JPanel getUpperFormPanel() {
        if (formPanel == null) {
            ...
            formPanel.setBorder(new TitledBorder(null, "Información del Proveedor",
    ...
    private void readAndShowAll(boolean showSize0Error) {
        try {
            List<Vendor> brsL = MemoryRepository.getInstance().readAll(Vendor.class);
            editingPrimaryId = -1;
    ...
    private void populateSelectedRowInForm(int selectedId) {
        try {
            Vendor bro = (Vendor) MemoryRepository.getInstance().getById(Vendor.class, selectedId);
            if (bro != null) {
